"""Create the ``agno`` schema for Agno-owned tables.

Agno's :class:`agno.db.postgres.PostgresDb` stores its own bookkeeping
(``agno_sessions``, ``agno_memories``, ``agno_runs``, …) in a
configurable schema.  We pin it to ``agno`` so those tables stay
isolated from our application tables in ``ai`` — but we set
