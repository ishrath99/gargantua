...
DPA_BASE = os.environ.get("DPA_BASE_URL", "").strip() or "https://10.4.41.188:8124"
...
def _get(path: str, params: dict | None = None, _attempt: int = 0) -> Any:
    ...
    try:
        ...
        if e.code in (302, 401):
            if _attempt >= _MAX_AUTH_RETRIES:
                raise RuntimeError(
                    f"DPA authentication failed after {_MAX_AUTH_RETRIES} retries "
...
def _post(path: str, payload: dict, _attempt: int = 0) -> Any:
    ...
    try:
        ...
        if e.code in (302, 401):
            ...
            _login()
            return _post(path, payload, _attempt + 1)
        raise RuntimeError(f"HTTP {e.code} from {path}: {e.read().decode()[:200]}")
...
@mcp.tool()
def get_index_advices(db_id: int, day: str) -> dict:
    """
    Return index advice details for a given day from the index advisor (/ia).
    Provides detailed index recommendations including SQL statements, estimated
    improvements, and advisor-specific metadata.

    Args:
        db_id: The numeric database ID (e.g. 1 or 2).
        day:   Date in YYYY-MM-DD format, e.g. "2026-03-10".
               Use get_tuning_dates to find days with index advice.
    """
    return _get(f"/iwc/rest/databases/{db_id}/ia/index-advices", {"day": day})
